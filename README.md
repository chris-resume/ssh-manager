...
* SSH connection with passwordless `sudo` without modifying the remote server; the password on the remote server is also not visible in the process name (for example in `top`).
  ...
  * If "Bash shell" is configured to be `true`, uses Bash *rcfile* file descriptor with the following printed out:
```
...
```
    * Additionally, if "passwordless sudo" is also configured to be `true`, the following extra functions are also declared, which take a username as an argument, defaulting to `root`:
